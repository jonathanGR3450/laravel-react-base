...
const InfoDetallesPredio = () => {
  ...
  const secciones = [
    ...
    {
      componente: <InfoConstrucciones />,
      alias: "Información de Construcciones",
    },
    {
      componente: <InfoUnidadConstruccion />,
      alias: "Información de Unidades de Construccion",
    },
  ...
  return (
    <div className=" flex flex-col w-full p-4 font-bold uppercase cursor-pointer ">
      {secciones.map((seccion, index) => (
        <div
          key={index}
          className="acordeon-seccion mb-1 bg-slate-100  bg-opacity-70 p-3 border hover:bg-slate-200 transition-colors"
        >
          <div
          ...
          {index === acordeonAbierto && (
            <div className="acordeon-contenido m-5 border p-2 bg-white bg-opacity-80 ">
              {seccion.componente}
            </div>
          )}
        </div>
