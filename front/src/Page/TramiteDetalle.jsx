  ...
  const interesadoCancela = [];
    for (let i = 0; i < cancela.data.Predio.length; i++) {
        for (var j = 0; j < cancela.data.Predio[i].derechos.length; j++) {
            interesadoCancela.push(cancela.data.Predio[i].derechos[j].interesado_lc_interesado)
        }
    }
...
//  let fuenteAdministrativa = dataDesenglobe[0].fuente_administrativa;
  ...
  for (let i = 0; i < cancela.data.Predio.length; i++) {
      for (var j = 0; j < cancela.data.Predio[i].derechos.length; j++) {
        for (var k = 0; k < cancela.data.Predio[i].derechos[j].fuenteadministrativa.length; k++) {
          fuenteAdministrativaCancela.push(cancela.data.Predio[i].derechos[j].fuenteadministrativa[k]);
        }
      }
  }
  ...
  function handleInput(e) {          
    ...
    function handleEnviar(e) {  
        ...
        if(!(ref.current.value) || !(ref1.current.value) || !(ref2.current.value) || !(ref3.current.value) || !(ref4.current.value)){
                ...
                <AccordionItemPanel>
                    ...
                    <div className=" flex justify-center items-center w-full mb-3">
                        ...
                        <div className="w-1/4 flex flex-col  ml-4 ">
                            ...
                            >                            
                                {
                                mutacionJson.mutacion.map((registro, key) => <option value={registro.t_id}>{registro.dispname}</option>)
                                }                            
                            </select>                        
                        </div>          
                        <div className="w-1/4 flex flex-col  ml-4 ">
